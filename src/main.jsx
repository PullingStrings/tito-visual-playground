...
function MotionExperiment({motion,index='01',title='SAMSUNG SUPPORT',subtitle='A CLEARER PATH TO HELP.',image=contactUs,alt='Samsung Contact Us interface',mirror=false,interruption=['HELP /','MADE','CLEAR.']}){
 ...
 return <div className={`choreo ${mirror?'choreo--mirror':''}`} ref={root}><div className="choreo__pin"><div className="choreo__kicker"><span>{index} / LIVE STRESS TEST</span><span>GSAP / SCRUBBED / PINNED</span></div><div className="choreo__title-wrap"><h3 className="choreo__title">{title}</h3></div><p className="choreo__subtitle">{subtitle}</p><div className="choreo__stage"><div className="choreo__number" aria-hidden="true">{index}</div><div className="choreo__media"><div className="choreo__media-inner"><img src={image} alt={alt}/><div className="choreo__ghost" aria-hidden="true">{index}</div><div className="choreo__accent"/></div><div className="choreo__dot" aria-hidden="true"/></div><div className="choreo__interruption">{interruption.map(line=><span key={line}>{line}</span>)}</div></div></div><div className="choreo__captions"><div className="choreo__caption"><span>01 / SAME GRAMMAR</span><p>Signal reveal, parallax separation and editorial resolve stay consistent across projects.</p></div><div className="choreo__caption"><span>02 / DIRECTIONAL RHYTHM</span><p>{mirror?'This project mirrors the system: number, reveal origin and interruption bias all come from the left.':'This project establishes the right-biased version of the system.'}</p></div><div className="choreo__caption"><span>03 / ACCENT CONTROL</span><p>{mirror?'The source artwork is louder, so Tito’s orange accent is quieter and mostly structural.':'The quieter support UI gives the orange system more room to speak.'}</p></div></div></div>
}
